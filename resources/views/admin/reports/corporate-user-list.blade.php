...
<div class="card card-custom">
    <div class="card-header flex-wrap border-0 pt-6 pb-0">
        <div class="card-title">
            <h3 class="card-label">Corporate Users List
                <div class="text-muted pt-2 font-size-sm"></div>
            </h3>
        </div>
        <div class="card-toolbar">
            <div class="dropdown dropdown-inline mr-2"></div>
            {{-- <a href="{{ url('admin/create/corporate-admin') }}" class="btn btn-primary font-weight-bolder mr-2">
                <i class="icon-2x text-white-50 flaticon-users-1"></i> Add Corporate Admin
            </a> --}}
...
@section('scripts')
    ...
    $(function () {
            ...
            $.ajax({
                ...
                success: function(data) {
                    $('<div class="pull-right">' +
                        '<select class="custom-select custom-select-sm form-control form-control-sm form-control-solid" id="searchByCorporate">'+data.data+'</select>' + '</div>').appendTo(".customDropDown");
                    $("#searchByCorporate").select2();
                    $(".select2-container").css('width', '100%');
                }
            });
            },
            dom: "<'row'<'col-sm-4'l><'col-sm-1'><'col-sm-3 customDropDown'><'col-sm-3'f>  >" +
                "<'row'<'col-sm-12'tr>>" +
            ...
            "order": [[ 0, "DESC" ]],
            headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            //ajax: "{{ route('corporateadmin.list') }}",
            ajax: {
                    url:"{{ route('allcorporateuser.list') }}",
                    data: function(data){
                        data.corporate_id = $('#searchByCorporate').val();
                    }
                },
                columns: [
    ...
    $('body').on('change', '#searchByCorporate', function () {
	      table.draw();
	});
